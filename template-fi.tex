...
Valinnan suorittaminen mahdollisimman aikaisin ei kuitenkaan aina tuota parasta tulosta. \cite{ramakrishnan2003database} Mikäli taulut on indeksoitu, voi valinta kadottaa tärkeät indeksit jolloin join-operaation suorittaminen hidastuu. Paras kyselysuunnitelma riippuu siten täysin taulujen koosta ja niiden indeksoinnista.

Optimoija voi suorittaa usean peräkkäisen join-operaation liittämisen monella eri tavalla. Tutkitaan kyselyä $A \Join B \Join C \Join D$. Vasemmalta syväksy (left-deep) puuksi kutsutaan tapaa liittää kyselyt seuraavasti: $(((A \Join B) \Join C) \Join D)$, jossa oikea lapsi on aina kantataulu. Optimoijat käyttävät usein pelkästään vasemmalta syviä puita, sillä eri tapoja liittää join-operaatioita on join-operaatioiden määrän kasvaessa liikaa. Toinen syy on vasemmalta syvien puiden mahdollistama putkitus. \cite{ramakrishnan2003database} Kuva 5 havainnollistaa vasemmalta syvät ja lehtevät puut.
